...
function Header({ onHome }) {
  return <header className="topbar"><Logo /><div className="topbar-center"><span className="live-dot" /> <span>WORKSPACE / GOA 2026</span><span className="slash">/</span><span className="muted">TASK 02</span></div><div className="topbar-actions"><div className="status-pill"><span className="pulse-dot" /> PIPELINE READY</div><button className="text-button topbar-home" onClick={onHome}>Home</button><button className="icon-button" aria-label="Help"><HelpCircle size={17} /></button><button className="avatar" aria-label="Profile">A</button></div></header>;
}

function LandingPage({ onEnter }) {
  return <main className="landing-page"><header className="landing-nav"><Logo /><div><span className="landing-nav-status"><span className="pulse-dot" /> LIVE RAG PIPELINE</span><button onClick={onEnter}>Open workbench <ArrowUpRight size={15} /></button></div></header><section className="landing-hero"><div className="landing-copy"><span className="landing-eyebrow"><Waves size={15} /> VOICE-NATIVE RETRIEVAL</span><h1>Turn a spoken question<br />into a <em>grounded answer.</em></h1><p>Voice RAG combines Sarvam transcription, retrieval profiles, guardrails, and streamed synthesis in one inspectable workspace.</p><div className="landing-actions"><button className="landing-primary" onClick={onEnter}>Enter the workbench <ArrowUpRight size={17} /></button><span>Task 02 / HH Goa 2026</span></div></div><div className="landing-instrument" aria-label="Voice RAG pipeline preview"><div className="instrument-orbit orbit-a" /><div className="instrument-orbit orbit-b" /><div className="instrument-core"><Mic2 size={34} /><span>VOICE RAG</span><b>LIVE</b></div><div className="instrument-node node-stt"><span>01</span><strong>STT</strong><em>Sarvam</em></div><div className="instrument-node node-retrieve"><span>02</span><strong>RETRIEVE</strong><em>Hybrid index</em></div><div className="instrument-node node-ground"><span>03</span><strong>GROUND</strong><em>Evidence check</em></div><div className="instrument-node node-answer"><span>04</span><strong>ANSWER</strong><em>Streamed output</em></div></div></section><section className="landing-proof"><div><span>VOICE → TEXT</span><b>Sarvam STT</b></div><div><span>TEXT → EVIDENCE</span><b>Four retrieval strategies</b></div><div><span>EVIDENCE → ANSWER</span><b>Live latency trace</b></div></section></main>;
}
...
function LiveExecutionPanel({ metrics, processing, phase, guardrail, onClearCache }) {
...
function QueryStage({ strategy, setStrategy }) {
  return <main className="query-stage"><div className="stage-kicker"><span className="signal-line" /> VOICE RAG WORKBENCH <span className="stage-id">/ LIVE</span></div><div className="hero-copy"><h1>Choose the<br /><span>retrieval layer.</span></h1><p>Select the chunking profile for the next voice or typed query. The supporting controls now live beside your response log.</p></div><section className="query-panel panel"><div className="query-panel-header"><div><span className="eyebrow"><Sparkles size={13} /> CHUNKING STRATEGY</span><h2>Choose your retrieval profile</h2></div><span className="recall-badge">Backend <b>connected</b></span></div><div className="strategy-tabs" role="tablist">{[["fixed_128", "Fixed-128", "Low latency"], ["fixed_256", "Fixed-256", "Balanced"], ["semantic", "Semantic", "Paragraphs"], ["sentence_window", "Window", "Sentence"]].map(([value, label, detail]) => <button key={value} className={strategy === value ? "selected" : ""} onClick={() => setStrategy(value)} role="tab" aria-selected={strategy === value}><strong>{label}</strong><span>{detail}</span></button>)}</div><div className="strategy-note"><Zap size={14} /><strong>Live pipeline</strong><span> Backend response timings populate the dashboard.</span><AlertTriangle size={14} /><span className="warning-copy">Values appear after the first request.</span></div></section></main>;
}

function WorkspaceControls({ onSubmit, onMicToggle, isListening, isProcessing, error, strategy, transcript, sttStatus, metrics }) {
...
export default function App() {
  ...
  if (view === "landing") return <LandingPage onEnter={() => setView("workspace")} />;

  return <div className="slide-deck"><section className="slide-scene slide-one"><div className="app-shell"><Header onHome={() => setView("landing")} /><div className="dashboard-layout"><aside className="left-workspace-rail"><HistoryRail messages={messages} onClear={() => setMessages([])} guardrail={guardrail} transcript={transcript} sttStatus={sttStatus} /><WorkspaceControls onSubmit={runQuery} onMicToggle={toggleMicrophone} isListening={isListening} isProcessing={isProcessing} error={error} strategy={strategy} transcript={transcript} sttStatus={sttStatus} metrics={metrics} /></aside><div className="main-workspace"><QueryStage strategy={strategy} setStrategy={setStrategy} /></div><aside className="telemetry-rail"><LiveExecutionPanel metrics={metrics} processing={isProcessing} phase={pipelinePhase} guardrail={guardrail} onClearCache={resetCache} /><LiveAnalytics metrics={metrics} latencyHistory={latencyHistory} /></aside></div><footer className="bottom-status"><span><span className="live-dot" /> Live backend environment</span><span>{metrics.retrievalMethod || "Awaiting first request"}</span><span>{formatMs(metrics.endToEndLatencyMs ?? metrics.streamRoundTripLatencyMs ?? metrics.serverTotalMs ?? metrics.totalLatencyMs)}</span></footer></div></section><section className="slide-scene slide-two"><LatencySequence metrics={metrics} /></section><AnswerDock answer={latestAnswer} metrics={metrics} isOpen={answersOpen} setIsOpen={setAnswersOpen} /></div>;
}
