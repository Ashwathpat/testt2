...
const formatMs = (value) => numberOrNull(value) ? `${Math.round(numberOrNull(value))} ms` : "—";
const metricValue = (value) => numberOrNull(value) ? Math.round(numberOrNull(value)) : "—";
...
function LiveExecutionPanel({ metrics, processing, phase, guardrail }) {
  const endToEnd = metrics.serverTotalMs || metrics.totalLatencyMs;
  const steps = [
...
export default function App() {
  ...
  const runQuery = async (query, selectedStrategy, sttLatencyMs = 0) => {
    ...
    setMetrics((current) => ({ ...current, sttLatencyMs: numberOrNull(sttLatencyMs), retrievalLatencyMs: null, groundingLatencyMs: null, generationLatencyMs: null, totalLatencyMs: null, serverTotalMs: null, ttftMs: null }));
    try {
      ...
      const total = numberOrNull(liveMetrics.serverTotalMs || liveMetrics.totalLatencyMs);
